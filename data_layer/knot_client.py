...
def _auth() -> HTTPBasicAuth:
    ...
    if not cid or not sec:
        raise RuntimeError("Set KNOT_CLIENT_ID and KNOT_SECRET in .env (dev keys, no quotes/spaces)")
...
def get_merchant_accounts(external_user_id: str, product_type: str = "transaction_link") -> List[Dict[str, Any]]:
    ...
    r = requests.get(f"{BASE}/accounts/get", params=params, auth=_auth(), headers=_headers(), timeout=TIMEOUT)
    if r.status_code == 401:
        raise RuntimeError(f"Knot 401 Unauthorized. Body: {r.text}")
    r.raise_for_status()
    return r.json()
